...
export function UserProvider({ children }) {
  ...
  const loadData = useCallback(async () => {
    ...
    const me = formattedUsers.find(u => u.id === myId);
    ...
    if (exError) {
      ...
      const formattedExchanges = dbExchanges.map(e => {
        ...
        return {
          ...
          status: e.status,
          you: { 
            name: iAmUser1 ? e.user1?.name : e.user2?.name, 
            offering: iAmUser1 ? e.user1_offering : e.user2_offering 
          },
          partner: { 
            name: iAmUser1 ? e.user2?.name : e.user1?.name, 
            offering: iAmUser1 ? e.user2_offering : e.user1_offering, 
            avatar: iAmUser1 ? e.user2?.avatar : e.user1?.avatar 
          },
