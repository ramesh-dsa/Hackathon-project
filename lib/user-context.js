...
export function UserProvider({ children }) {
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: dbExchanges, error: exError } = await supabase
      .from('exchanges')
      .select(`
        id, status, user1_id, user2_id, user1_offering, user2_offering, last_updated, user1_completed, user2_completed,
    ...
    if (exError) {
      ...
      const formattedExchanges = dbExchanges.map(e => {
        ...
        return {
          id: e.id,
          status: e.status,
          myCompleted: iAmUser1 ? e.user1_completed : e.user2_completed,
          partnerCompleted: iAmUser1 ? e.user2_completed : e.user1_completed,
          isUser1: iAmUser1,
          you: { 
            name: me?.name, 
  ...
  useEffect(() => {
    ...
    const usersSubscription = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'requests' }, () => {
        // Trigger a full reload when requests change (since we need joined user data)
        loadData();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'exchanges' }, () => {
        // Trigger a full reload when exchanges change
        loadData();
      })
      .subscribe();
  ...
  const completeAndReviewExchange = async (exchangeId, partnerId, rating, reviewText) => {
    // 1. Fetch current exchange to determine user role and partner completion status
    const { data: exc, error: fetchErr } = await supabase
      .from('exchanges')
      .select('user1_id, user2_id, user1_completed, user2_completed')
      .eq('id', exchangeId)
      .single();

    if (fetchErr || !exc) {
      console.error("Failed to fetch exchange:", fetchErr);
      return;
    }

    const isUser1 = exc.user1_id === currentUser.id;
    const updateField = isUser1 ? 'user1_completed' : 'user2_completed';
    const partnerCompleted = isUser1 ? exc.user2_completed : exc.user1_completed;
    
    // If partner already completed, this makes both complete, so update status to 'completed'
    const newStatus = partnerCompleted ? 'completed' : 'in-progress';

    // 2. Mark your part of the exchange as completed
    const { error: excError } = await supabase
      .from('exchanges')
      .update({ [updateField]: true, status: newStatus, last_updated: 'Just now' })
      .eq('id', exchangeId);

    if (excError) {
      console.error("Failed to complete exchange:", excError);
      return;
    }
    ...
    if (rating && partnerId) {
      ...
      if (revError) {
        console.error("Failed to insert review:", revError);
      }
    }
    ...
    // Trigger a refresh of the user data to reflect new reviews
    loadData(); 
  };
  ...
  return (
    <UserContext.Provider
      value={{ 
        ...
        sendRequest, acceptRequest, declineRequest, cancelRequest, completeExchange, completeAndReviewExchange,
