...
export function UserProvider({ children }) {
        ...
        setAllUsers((prevUsers) => {
          return prevUsers.map(u => {
            if (u.id === payload.new?.user_id || u.id === payload.old?.user_id) {
              ...
              // We need to fetch the updated skills, but since it's realtime,
              // we can just push or pull from the array based on INSERT/DELETE.
              if (payload.eventType === 'INSERT') {
                const newSkill = payload.new.skill_name;
                const type = payload.new.type;
                if (type === 'offer' && !u.offers.includes(newSkill)) {
                  return { ...u, offers: [...u.offers, newSkill] };
                } else if (type === 'need' && !u.needs.includes(newSkill)) {
                ...
                if (type === 'offer') {
                  return { ...u, offers: u.offers.filter(s => s !== oldSkill) };
                } else if (type === 'need') {
                  return { ...u, needs: u.needs.filter(s => s !== oldSkill) };
                }
              }
            }
            return u;
  ...
  const addOffer = async (skill) => {
    ...
    const newOffers = [...(currentUser?.offers || []), skill];
  ...
  const removeOffer = async (skill) => {
    ...
    const newOffers = (currentUser?.offers || []).filter(s => s !== skill);
    setCurrentUser(prev => ({ ...prev, offers: newOffers }));
    setAllUsers(prev => prev.map(u => u.id === currentUser.id ? { ...u, offers: newOffers } : u));
  };
  ...
  const addNeed = async (skill) => {
    ...
    const newNeeds = [...(currentUser?.needs || []), skill];
  ...
  const removeNeed = async (skill) => {
    ...
    const newNeeds = (currentUser?.needs || []).filter(s => s !== skill);
    setCurrentUser(prev => ({ ...prev, needs: newNeeds }));
    setAllUsers(prev => prev.map(u => u.id === currentUser.id ? { ...u, needs: newNeeds } : u));
  };
